...
class NotificacoesUserActivity : AppCompatActivity() {
    ...
    // Adapter com ação ao clicar numa notificação
    ...
    // Carrega notificações da API e filtra apenas as do utilizador atual
            ...
            try {
                ...
                if (response.isSuccessful) {
                    response.body()?.let { notificacoes ->
                        ...
                        Toast.makeText(this@NotificacoesUserActivity, R.string.toast_nenhuma_notificacao, Toast.LENGTH_SHORT).show()
                    }
                } else {
                    Toast.makeText(this@NotificacoesUserActivity, "Erro ao carregar notificações: ${response.code()}", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                Log.e("NotificacoesUser", "Erro ao carregar notificações: ${e.message}")
                Toast.makeText(this@NotificacoesUserActivity, "Erro: ${e.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
}